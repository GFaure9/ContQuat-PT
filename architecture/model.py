...
from .prepro_layers_for_LsBERTCon import SBERTLossPreproLayers
...
class Model(nn.Module):
    ...
    def forward(
        ...
        return skel_out, gloss_out, decoder_self_attention_outputs
    ...
    def decode(
        ...
        decoder_output = self.decoder(
            trg_embed=trg_embed, encoder_output=encoder_output, src_mask=src_mask, trg_mask=trg_mask
        )
    ...
    def get_loss_for_batch(self, batch: Batch, loss_function: nn.Module) -> Tensor:
        ...
        if isinstance(loss_function, LossWithSBERTCont):
            ...
            if self.prepro_layers_for_LsBERTCon is not None:
                # checking that the output size of the FFN equals the dimension of sentences embeddings
                assert self.prepro_layers_for_LsBERTCon.linear.out_features == batch.sent_emb.shape[-1]
                sbert_cont_inputs = torch.stack(
                    [self.prepro_layers_for_LsBERTCon(tens) for tens in sbert_cont_inputs], dim=0
                )  # shape=(num_self_att_layer, N_batch, dim_sentence_embedding)
...
def build_model(
    ...
    decoder = TransformerDecoder(
        **cfg["decoder"],
        ...
        trg_size=transformer_decoder_out_size,
    ...
    model = Model(
        ...
        sent_emb_size=sent_emb_size  # for when L_sBERTCont is computed with prepro layers before
    )

    # Custom initialization of model parameters
    initialize_model(model, cfg, src_padding_idx)
