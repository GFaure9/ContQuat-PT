...
from ..utils.helpers import freeze_params, ConfigurationError, subsequent_mask, uneven_subsequent_mask
...
class TransformerDecoder(Decoder):
    ...
    def __init__(self,
                 ...
                 apply_root_quat_treatment: bool = False,  # added by me (GF)
                 **kwargs):
        """
        ...
        # :param use_quaternions: whether to predict poses in quaternion representation before transforming back to 3D coordinates
        ...
        # OR root joint vector + 4 * N_bones + 1 (comment added by GF)
        self.output_layer = nn.Linear(hidden_size, trg_size, bias=False)  # original
    ...
    def forward(self,
        ...
        decoder_self_attention_outputs = torch.zeros(len(self.layers), x.shape[0], x.shape[1], self._hidden_size, device=x.device)  # added by me (GF)

        for k, layer in enumerate(self.layers):
            x, h1 = layer(x=x, memory=encoder_output,
                      src_mask=src_mask, trg_mask=sub_mask, padding_mask=padding_mask)  # `h1` added by me (GF)
            decoder_self_attention_outputs[k] = h1  # added by me (GF)
        ...
        if self.apply_root_quat_treatment:
            ...
            return output_new, x, decoder_self_attention_outputs, None  # `decoder_self_attention_outputs` added by me (GF) for contrastive losses computations
        else:
            return output, x, decoder_self_attention_outputs, None

        # todo(gf): [remark for contrastive loss]
        #           The tensor of self-attention hidden states should be returned instead of the 3rd None for example.
        #           `encoder_output` should also be returned, instead of the 4th None for example (not sure).
